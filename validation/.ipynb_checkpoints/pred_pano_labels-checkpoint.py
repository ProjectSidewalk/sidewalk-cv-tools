...
def make_sliding_window_crops(pano_id, GSV_IMAGE_WIDTH, GSV_IMAGE_HEIGHT, path_to_gsv_scrapes, num_threads=4, verbose=False):
    ...
    for i in range(0, num_threads):
        c_threads.append(threading.Thread(target=crop_threader, args=(crop_queue,deptvh,pano_img.copy(),i,quit_cropping,verbose, )))
        c_threads[i].start()
...
if __name__ == '__main__':
    #print test_singlecrops("1a1UlhadSS_3dNtc5oI10Q", "panos/", 13312, 6656, "models/",num_threads=12, save_labeled_pano=True, verbose=True)
    print(single_crops("single/", "single", "models/", verbose=True))
