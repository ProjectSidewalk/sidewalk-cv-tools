...
def read_validation_data(path, date_after, existing_labels, add_to_summary, number_agree):
	dict = {}
	updated ={}
	totalcount = 0
...
def generate_data(input_data, date_after,path_to_panos, ignore_null, number_agree, path_to_summary):
	utils.clear_dir("single\\crops")
	existing_labels = labels_already_made(path_to_panos)
	add_to_summary = {}
	dict_valid = read_validation_data(input_data, date_after, existing_labels, add_to_summary, number_agree)
	if(len(dict_valid) == 0):
		return
...
def generate_results_data(input_data,path_to_panos,path_to_summary, number_agree = 2,date_after = "2018-06-28"):
	if not os.path.isdir(path_to_panos):
		print("There is no such directory for path to panos")
		return False
	first = time.time()
	ignore_null = True 
	generate_data(input_data, date_after, path_to_panos, ignore_null, number_agree, path_to_summary)
	second = time.time()
