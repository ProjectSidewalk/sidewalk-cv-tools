	...
	if os.path.exists(path_to_completelabels):
		with open(path_to_completelabels, 'r') as csvfile: 
			...
			for row in csvreader: 
				...
				percentage = round(100.0 * (float(row[max]) + 10.0)/20.0,2)
...
user_info = {}
def write_user_summary_file(rows_dict, labels_list , add_to_summary, path_to_summary):
	new_lables = []
	name_of_summaryfile = os.path.join(path_to_summary,"user_summary.csv")
	if os.path.exists(name_of_summaryfile):
		os.remove(name_of_summaryfile)
	with open(name_of_summaryfile, 'w+', newline='') as csvfile:
			writer = csv.writer(csvfile)
			for first, second in add_to_summary.items():
				pano_id, sv_x, sv_y = first.split(",")
				cvlabel, userlabel, confidence = second.split(",")
				complete = pano_id + "," + str(float(sv_x)) + "," + str(float(sv_y))
				for user in user_info[complete]:
					userlabel,label_id,user_id = user.split(",")
					x = int(float(sv_x))
					y = int(float(sv_y))
					row = [pano_id, x, y, cvlabel, userlabel, confidence, label_id, user_id]
					writer.writerow(row)
			labelrowcount = 0
			for labelrow in labels_list:
				pano_id = labelrow[0]
				...
				if(complete in rows_dict and complete not in add_to_summary):
					cv_respone = rows_dict[complete]
					label,confidence  = cv_respone.split(",")
					value = [pano_id, x, y] + [label,confidence]
					copy = value.copy()
					for user in user_info[complete]:
						cvlabel,label_id,user_id = user.split(",")
						value.insert(4,cvlabel)
						value.extend((label_id,user_id))
						writer.writerow(value)
					new_lables.append(value)
					labelrowcount += 1
	return new_lables
...
def read_user_data(path, date_after, existing_labels, add_to_summary, number_agree, verbose):
	global user_info
	dict = {}
	updated ={}
	totalcount = 0
		...
		with open(path) as csvfile: 
			...
			for row in csvreader:
				...
				if(date > date_after):
					...
					label_id = row[5]
					user_id = row[6]
					user_total = label + "," + label_id + "," + user_id
					if not(complete in dict):
						dict[complete] = [complete in existing_labels, label]
						user_info[complete] = [user_total]
					else:
						dict[complete].append(label)
						user_info[complete].append(user_total)
			counter = 0
			for key,predictions in dict.items():
				count = [0, 0, 0, 0, 0]
				counter += 1
...
def generate_user_data(input_data, date_after,path_to_panos, ignore_null, number_agree, path_to_summary, verbose, num_threads):
	crops = os.path.join("single","crops")
	if not os.path.exists(crops):
		os.makedirs(crops)
	utils.clear_dir(crops)
	existing_labels = labels_already_made(path_to_panos)
	add_to_summary = {}
	dict_valid = read_user_data(input_data, date_after, existing_labels, add_to_summary, number_agree, verbose)
	dict_image = generate_image_date(dict_valid, existing_labels, verbose)
	make_crop_threading(dict_image, path_to_panos, verbose, num_threads) 
	get_results(verbose)
	rows_dict = exact_labels(ignore_null)
	labels_list = generate_labelrows(dict_valid)
	new_labels = write_user_summary_file(rows_dict, labels_list, add_to_summary, path_to_summary)
	if(verbose):
		print("Number of new labels is " + str(len(new_labels)))
	update_labels_already_made(new_labels,path_to_panos)
	utils.clear_dir(crops)
...
def generate_validation_userdata(input_data,path_to_panos,path_to_summary, number_agree = 1,num_threads = 4, date_after = "2018-06-28", verbose = False):
	...
	first = time.time()
	ignore_null = True 
	generate_user_data(input_data, date_after, path_to_panos, ignore_null, number_agree, path_to_summary, verbose, num_threads)
	second = time.time()
	if(verbose):
		print("Program took: " + str((second - first)/60.0) + " minutes")
	return (os.path.join(path_to_summary,"user_summary.csv"))
